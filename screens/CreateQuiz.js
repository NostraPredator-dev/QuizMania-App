...
export default function CreateQuiz({ navigation }) {
    ...
    function fetchAvailableQuizTypes() {
        const quizTypesDbRef = db.ref('quizTypes');
        quizTypesDbRef.on("value", (snap)=> {
            var quizTypes = snap.val();
            var typeList = [];
            console.log(quizTypes)
            for (var x in quizTypes)
            {
                console.log(quizTypes[x])
                typeList.push(quizTypes[x])
            }
